...
public class ServerOAuth2LoginAuthenticationTokenConverter implements ServerAuthenticationConverter {
	...
	private Mono<OAuth2LoginAuthenticationToken> authenticationRequest(ServerWebExchange exchange, OAuth2AuthorizationRequest authorizationRequest) {
		return Mono.just(authorizationRequest)
				...
				.map(clientRegistration -> {
					OAuth2AuthorizationResponse authorizationResponse = convertToResponse(exchange);
	...
	private static OAuth2AuthorizationResponse convertToResponse(ServerWebExchange exchange) {
