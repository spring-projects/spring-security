...
public class WithMockOidcIdTokenSecurityContextFactoryTests {
	...
	@WithMockOidcIdToken(
			...
			claims = @StringAttribute(name = OAuth2IntrospectionClaimNames.SCOPE, value = "a b", parser = SpacedSeparatedStringsParser.class))
	...
	@WithMockOidcIdToken(
			...
			claims = {
					@StringAttribute(name = OAuth2IntrospectionClaimNames.SCOPE, value = "a b", parser = SpacedSeparatedStringsParser.class),
					@StringAttribute(
	...
	public void customFrequent() {
		...
		assertThat(auth.getAuthorities())
				.containsExactlyInAnyOrder(adminRole, userRole, scopeAAuthority, scopeBAuthority);
	...
	public void customFull() throws Exception {
		...
		assertThat((Collection<String>) authorizationRequest.getAttributes().get("scope")).containsExactlyInAnyOrder("a", "b");
