...
public class OpenSamlLogoutResponseResolverTests {
	...
	public void resolveRedirectWhenAuthenticatedThenSuccess() {
		...
		OpenSamlLogoutResponseResolver logoutResolver = new OpenSamlLogoutResponseResolver(
				(request, id) -> registration);
		MockHttpServletRequest request = new MockHttpServletRequest();
		LogoutRequest logoutRequest = TestOpenSamlObjects.assertingPartyLogoutRequest(registration);
		request.setParameter("SAMLResponse", OpenSamlSigningUtils.serialize(logoutRequest));
		Authentication authentication = authentication(registration);
		Saml2LogoutResponse saml2LogoutResponse = logoutResolver.resolveLogoutResponse(request, authentication)
				.logoutResponse();
