...
class BasicSsrfProtectionFilter implements SsrfProtectionFilter {
	...
	public InetAddress[] filterAddresses(InetAddress[] addresses) throws HostBlockedException {
		...
		if (result.isEmpty()) {
			...
			throw new HostBlockedException(
					String.format("The following address(es) were blocked due to violating %s, policy: %s", mode.name(),
							addrFmt));
		}
