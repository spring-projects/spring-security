...
Spring Boot users may use `server.forward-headers-strategy` or `server.use-forward-headers` property to configure application behind proxy server (details could be found at https://docs.spring.io/spring-boot/docs/2.2.0.BUILD-SNAPSHOT/reference/html/howto.html#howto-use-tomcat-behind-a-proxy-server[Spring Boot documentation]).
