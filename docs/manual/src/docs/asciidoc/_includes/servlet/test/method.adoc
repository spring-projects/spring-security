...
[[test-method-oauth2]]
=== Test OAuth2 secured resource-servers

Various annotations are also provided to setup your tests `SecurityContext` with OAuth2 authentications. Depending on your application needs, you can pick one of:
  * `@WithMockJwt`: Authentication is a `JwtAuthenticationToken`, principal a `Jwt`
  * `@WithMockAccessToken`: Authentication is an `OAuth2IntrospectionAuthenticationToken`, principal an `OAuth2AccessToken`
  * `@WithMockOidcIdToken`: Authentication is an `OAuth2LoginAuthenticationToken`, principal a `DefaultOidcUser`

Usage are quite similar to `@WithMockUser`, each just offers additional properties to configure specific principal and authentication.

Short complete demo with `@WithMockJwt`:
[source,java]
----
@RunWith(SpringJUnit4ClassRunner.class)
