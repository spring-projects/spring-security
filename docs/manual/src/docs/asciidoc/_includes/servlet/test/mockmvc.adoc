...
==== Testing OAUth2 resource servers

In addition to mock user authentication, it is possible to easily mock three different kind of OAuth2 authentication:
  * `jwt()` populates SecurityContext with a `JwtAuthenticationToken`, principal being a `Jwt`
  * `accessToken()` populates SecurityContext with an `OAuth2IntrospectionAuthenticationToken`, principal being an `OAuth2AccessToken`
  * `oidcId()` populates SecurityContext with an `OAuth2LoginAuthenticationToken`, principal being a `DefaultOidcUser`

Each of above are highly configurable through flow APIs and are used just as other `MockMvc` `RequestPostProcessor`s:
...
----
mvc
	.perform(get("/").with(accessToken().name("ch4mpy"))
----
[source,java]
----
mvc
	.perform(get("/").with(oidcId().authority("SCOPE_message:read"))
----
