...
[NOTE]
====
The completed application can be found at {gh-samples-url}/javaconfig/helloworld[samples/javaconfig/helloworld]
====
// FIXME: Link to XML and Boot

== Updating Dependencies

The first step is to update the dependencies using <<getting-maven-without-spring-boot,Maven>> or <<gradle-without-spring-boot,Gradle>>.
