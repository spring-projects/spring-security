...
To customize validation, you can implement your own `Saml2LogoutRequestAuthenticator`.
At this point, the validation is minimal, so you may be able to first delegate to the default `Saml2LogoutRequestAuthenticator` like so:

[source,java]
----
@Component
public class MyOpenSamlLogoutRequestAuthenticator implements Saml2LogoutRequestAuthenticator {
	private final Saml2LogoutRequestAuthenticator delegate = new OpenSamlLogoutRequestAuthenticator();

	@Override
    public Saml2LogoutRequestAuthenticator logout(Saml2LogoutRequestAuthenticatorParameters parameters) {
		 // verify signature, issuer, destination, and principal name
		Saml2LogoutRequestAuthenticator result = delegate.authenticate(authentication);
...
Then, you can supply your custom `Saml2LogoutRequestAuthenticator` in the DSL as follows:
...
To customize validation, you can implement your own `Saml2LogoutResponseAuthenticator`.
At this point, the validation is minimal, so you may be able to first delegate to the default `Saml2LogoutResponseAuthenticator` like so:

[source,java]
----
@Component
public class MyOpenSamlLogoutResponseAuthenticator implements Saml2LogoutResponseAuthenticator {
	private final Saml2LogoutResponseAuthenticator delegate = new OpenSamlLogoutResponseAuthenticator();

	@Override
    public Saml2LogoutAuthenticatorResult logout(Saml2LogoutResponseAuthenticatorParameters parameters) {
		// verify signature, issuer, destination, and status
		Saml2LogoutAuthenticatorResult result = delegate.authenticate(parameters);
...
Then, you can supply your custom `Saml2LogoutResponseAuthenticator` in the DSL as follows:
