...
=== Storing the Authentication Request

The `Saml2AuthenticationRequestRepository` is responsible for the persistence of the `AbstractSaml2AuthenticationRequest` from the time the Authentication Request is initiated to the time the Authentication Response is received.
The `Saml2AuthenticationTokenConverter` is responsible for loading the `AbstractSaml2AuthenticationRequest` from the `Saml2AuthenticationRequestRepository` and saving it into the `Saml2AuthenticationToken`.

The default implementation of `Saml2AuthenticationRequestRepository` is `HttpSessionSaml2AuthenticationRequestRepository`, which stores the `AbstractSaml2AuthenticationRequest` in the `HttpSession`.
