...
OAuth2 Login does not use a resource server `JwtDecoder` bean to validate ID Tokens.
The following code shows how to configure the `JwtDecoderFactory<ClientRegistration>` and the HTTP timeouts used to retrieve the JWK Set:
...
public JwtDecoderFactory<ClientRegistration> idTokenDecoderFactory() {
	...
	OidcIdTokenDecoderFactory idTokenDecoderFactory = new OidcIdTokenDecoderFactory();
	idTokenDecoderFactory.setRestOperationsFactory((clientRegistration) -> restOperations);
	return idTokenDecoderFactory;
}
----
...
fun idTokenDecoderFactory(): JwtDecoderFactory<ClientRegistration> {
    ...
    val idTokenDecoderFactory = OidcIdTokenDecoderFactory()
    idTokenDecoderFactory.setRestOperationsFactory { restOperations }
    return idTokenDecoderFactory
}
----
======
