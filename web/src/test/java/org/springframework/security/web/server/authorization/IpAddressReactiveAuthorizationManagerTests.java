...
public class IpAddressReactiveAuthorizationManagerTests {
	...
	final IpAddressReactiveAuthorizationManager v4manager = IpAddressReactiveAuthorizationManager
			.hasIpAddress("192.168.1.104");

	final IpAddressReactiveAuthorizationManager v6manager = IpAddressReactiveAuthorizationManager
			.hasIpAddress("fe80::21f:5bff:fe33:bd68");

	@Test
	public void checkWhenHasIpv6AddressThenReturnTrue() throws UnknownHostException {
		boolean granted = this.v6manager.check(null, context("fe80::21f:5bff:fe33:bd68")).block().isGranted();
		assertThat(granted).isTrue();
	}

	@Test
	public void checkWhenHasIpv6AddressThenReturnFalse() throws UnknownHostException {
		boolean granted = this.v6manager.check(null, context("fe80::1c9a:7cfd:29a8:a91e")).block().isGranted();
		assertThat(granted).isFalse();
	}

	@Test
	public void checkWhenHasIpv4AddressThenReturnTrue() throws UnknownHostException {
		boolean granted = this.v4manager.check(null, context("192.168.1.104")).block().isGranted();
		assertThat(granted).isTrue();
	}

	@Test
	public void checkWhenHasIpv4AddressThenReturnFalse() throws UnknownHostException {
		boolean granted = this.v4manager.check(null, context("192.168.100.15")).block().isGranted();
		assertThat(granted).isFalse();
	}
