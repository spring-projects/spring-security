...
public class SwitchUserFilterTests {
    ...
    @Test
    public void exitUserJackLordToDanoWithExitHandlerSetSucceeds() throws Exception {
        // original user
        UsernamePasswordAuthenticationToken source = new UsernamePasswordAuthenticationToken(
                "dano", "hawaii50", ROLES_12);
        ...
        filter.setUserDetailsService(new MockUserDetailsService());
        filter.setExitUserUrl("/logout/impersonate");
        filter.setExitSuccessHandler(new SimpleUrlAuthenticationSuccessHandler(
                "/webapp/exitTarget"));
        ...
        assertEquals("/webapp/exitTarget", response.getRedirectedUrl());
    }

    @Test
    public void exitUserJackLordToDanoWithExitTargetUrlSetSucceeds() throws Exception {
        // original user
        UsernamePasswordAuthenticationToken source = new UsernamePasswordAuthenticationToken(
                "dano", "hawaii50", ROLES_12);
        ...
        filter.setExitTargetUrl("/webapp/exitTarget");
        filter.afterPropertiesSet();
