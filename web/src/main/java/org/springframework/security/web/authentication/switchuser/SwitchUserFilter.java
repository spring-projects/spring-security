 ...
 *    &lt;property name="exitTargetUrl" value="/switch/userList.jsp" />
 * &lt;/bean>
 * </pre>
 *
...
public class SwitchUserFilter extends GenericFilterBean implements
    ...
    private AuthenticationSuccessHandler exitSuccessHandler;
    ...
    public void afterPropertiesSet() {
        ...
        if (exitTargetUrl != null) {
            Assert.isNull(exitSuccessHandler,
                    "You cannot set both exitSuccessHandler and exitTargetUrl");
            exitSuccessHandler = new SimpleUrlAuthenticationSuccessHandler(exitTargetUrl);
        }

        //if not specific exist target handler is defined use target handler for backward compatibility
        if (exitSuccessHandler == null) {
            exitSuccessHandler = successHandler;
        }
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (requiresSwitchUser(request)) {
            ...
            // redirect to exit target url
            exitSuccessHandler.onAuthenticationSuccess(request, response, originalUser);
    ...
    /**
     * Sets the URL to go to after a successful exit user request. Use
     * {@link #setExitSuccessHandler(AuthenticationSuccessHandler) setExitSuccessHandler} instead
     * if you need more customized behaviour.
     *
     * @param exitTargetUrl The target url.
     */
    public void setExitTargetUrl(String exitTargetUrl) {
        Assert.isTrue(UrlUtils.isValidRedirectUrl(exitTargetUrl),
                "exitTargetUrl cannot be empty and must be a valid redirect URL");
        this.exitTargetUrl = exitTargetUrl;
    }

    /**
     * Used to define custom behaviour on a successful exit user.
     * <p>
     * Can be used instead of setting <tt>exitTargetUrl</tt>.
     */

    public void setExitSuccessHandler(AuthenticationSuccessHandler exitSuccessHandler) {
        Assert.notNull(exitSuccessHandler, "exitSuccessHandler cannot be null");
