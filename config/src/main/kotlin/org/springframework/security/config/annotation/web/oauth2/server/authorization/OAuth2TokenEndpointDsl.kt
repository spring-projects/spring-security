...
class OAuth2TokenEndpointDsl {
    ...
    var accessTokenRequestConverter: AuthenticationConverter? = null
    var accessTokenRequestConverters: Consumer<MutableList<AuthenticationConverter>>? = null
    ...
    internal fun get(): (OAuth2TokenEndpointConfigurer) -> Unit {
        return { tokenEndpoint ->
            accessTokenRequestConverter?.also { tokenEndpoint.accessTokenRequestConverter(it) }
            accessTokenRequestConverters?.also { tokenEndpoint.accessTokenRequestConverters(it) }
            authenticationProvider?.also { tokenEndpoint.authenticationProvider(it) }
            authenticationProviders?.also { tokenEndpoint.authenticationProviders(it) }
            accessTokenResponseHandler?.also { tokenEndpoint.accessTokenResponseHandler(it) }
            errorResponseHandler?.also { tokenEndpoint.errorResponseHandler(it) }
        }
    }
}
