 ...
 */
public class JdbcUserPasswordDetailsManagerBeanDefinitionParserTests {
	...
	public void beanNameIsCorrect() {
		assertThat(JdbcUserPasswordDetailsManager.class.getName())
			.isEqualTo(new JdbcUserPasswordDetailsManagerBeanDefinitionParser().getBeanClassName(mock(Element.class)));
	}
	...
	public void beanIdIsParsedCorrectly() {
		setContext("<jdbc-user-password-service id='myUserService' data-source-ref='dataSource'/>" + DATA_SOURCE);
		assertThat(this.appContext.getBean("myUserService") instanceof JdbcUserPasswordDetailsManager).isTrue();
	}
	...
	public void usernameAndAuthorityQueriesAreParsedCorrectly() throws Exception {
		...
		// @formatter:off
		setContext("<jdbc-user-password-service id='myUserService' "
	...
	public void isSupportedByAuthenticationProviderElement() {
		...
		setContext("<authentication-manager>"
				...
				+ "    <jdbc-user-password-service data-source-ref='dataSource'/>"
				+ "  </authentication-provider>"
	...
	public void cacheIsInjectedIntoAuthenticationProvider() {
		...
		setContext("<authentication-manager>"
				+ "  <authentication-provider>"
				+ "    <jdbc-user-password-service cache-ref='userCache' data-source-ref='dataSource'/>"
				+ "  </authentication-provider>"
				+ "</authentication-manager>"
				+ DATA_SOURCE
				+ USER_CACHE_XML);
	...
	public void rolePrefixIsUsedWhenSet() {
		setContext("<jdbc-user-password-service id='myUserService' role-prefix='PREFIX_' data-source-ref='dataSource'/>"
				+ DATA_SOURCE);
		JdbcUserPasswordDetailsManager mgr = (JdbcUserPasswordDetailsManager) this.appContext.getBean("myUserService");
