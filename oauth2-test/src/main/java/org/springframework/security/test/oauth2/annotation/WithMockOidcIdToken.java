 ...
 *   scopes = {"machin", "chose"}
 *   claims = {
 *     &#64;StringAttribute(name = IdTokenClaimNames.ACR, value = "bidule") })
 * &#64;Test
...
public @interface WithMockOidcIdToken {
	 ...
	 */
	String name() default OidcIdTokenAuthenticationBuilder.DEFAULT_AUTH_NAME;

	/**
	 * @return claim name for subscriber (user name). Default value is very likely to
	 * match your need.
	 */
	String nameAttributeKey() default OidcIdTokenAuthenticationBuilder.DEFAULT_NAME_KEY;
		...
		static class AnnotationOidcIdTokenAuthenticationBuilder
			...
			public AnnotationOidcIdTokenAuthenticationBuilder(final WithMockOidcIdToken annotation) {
				...
				clientRegistrationBuilder.authorizationGrantType(
						isEmpty(annotation.clientRegistration().authorizationGrantType()) ? null
								: new AuthorizationGrantType(annotation.clientRegistration().authorizationGrantType()));
				clientRegistrationBuilder
						.authorizationUri(nonEmptyOrNull(annotation.clientRegistration().authorizationUri()));
				clientRegistrationBuilder.clientAuthenticationMethod(
						isEmpty(annotation.clientRegistration().clientAuthenticationMethod()) ? null
								: new ClientAuthenticationMethod(
										annotation.clientRegistration().clientAuthenticationMethod()));
				clientRegistrationBuilder.clientId(nonEmptyOrNull(annotation.clientRegistration().clientId()));
				clientRegistrationBuilder.clientName(nonEmptyOrNull(annotation.clientRegistration().clientName()));
				clientRegistrationBuilder.clientSecret(nonEmptyOrNull(annotation.clientRegistration().clientSecret()));
				clientRegistrationBuilder.jwkSetUri(nonEmptyOrNull(annotation.clientRegistration().jwkSetUri()));
				clientRegistrationBuilder
						.redirectUriTemplate(nonEmptyOrNull(annotation.clientRegistration().redirectUriTemplate()));
				clientRegistrationBuilder.providerConfigurationMetadata(
						parsingSupport.parse(annotation.clientRegistration().providerConfigurationMetadata()));
				clientRegistrationBuilder
						.registrationId(nonEmptyOrNull(annotation.clientRegistration().registrationId()));
				clientRegistrationBuilder.scope(allScopes);
				clientRegistrationBuilder.tokenUri(nonEmptyOrNull(annotation.clientRegistration().tokenUri()));
				clientRegistrationBuilder.userInfoAuthenticationMethod(
						isEmpty(annotation.clientRegistration().userInfoAuthenticationMethod()) ? null
								: new AuthenticationMethod(
										annotation.clientRegistration().userInfoAuthenticationMethod()));
				clientRegistrationBuilder.userInfoUri(nonEmptyOrNull(annotation.clientRegistration().userInfoUri()));
				clientRegistrationBuilder
						.userNameAttributeName(nonEmptyOrNull(annotation.clientRegistration().userNameAttributeName()));

				authorizationRequestBuilder.authorizationRequestUri(
						nonEmptyOrNull(annotation.authorizationRequest().authorizationRequestUri()));
				authorizationRequestBuilder
						.authorizationUri(nonEmptyOrNull(annotation.authorizationRequest().authorizationUri()));
				authorizationRequestBuilder.clientId(nonEmptyOrNull(annotation.authorizationRequest().clientId()));
				authorizationRequestBuilder
						.redirectUri(nonEmptyOrNull(annotation.authorizationRequest().redirectUri()));
