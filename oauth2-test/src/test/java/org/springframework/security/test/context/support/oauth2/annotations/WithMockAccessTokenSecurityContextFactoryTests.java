...
public class WithMockAccessTokenSecurityContextFactoryTests {
	...
	@WithMockAccessToken(
			...
			authorities = "SCOPE_a",
			claims = {
					@Attribute(name = "sub", value = "ch4mpy"),
					@Attribute(
	...
	@WithMockAccessToken(
			...
			claims = {
					@Attribute(
							...
							value = "false",
							parseTo = TargetType.BOOLEAN),
					@Attribute(
							name = OAuth2IntrospectionClaimNames.AUDIENCE,
							value = "c",
							parseTo = TargetType.STRING_LIST),
					@Attribute(
							name = OAuth2IntrospectionClaimNames.AUDIENCE,
							value = "d",
							parseTo = TargetType.STRING_LIST),
					@Attribute(name = OAuth2IntrospectionClaimNames.CLIENT_ID, value = "test-client"),
					@Attribute(
					...
					@Attribute(
							...
							parseTo = TargetType.INSTANT),
					@Attribute(name = OAuth2IntrospectionClaimNames.ISSUER, value = "test-issuer"),
					@Attribute(name = OAuth2IntrospectionClaimNames.JTI, value = "test ID"),
					@Attribute(
							name = OAuth2IntrospectionClaimNames.NOT_BEFORE,
							value = "2019-02-03T14:00:42.00Z",
							parseTo = TargetType.INSTANT),
					@Attribute(name = OAuth2IntrospectionClaimNames.SUBJECT, value = "test-subject") })
	...
	public void defaults() {
		...
		assertThat(auth.getName()).isEqualTo(WithMockAccessToken.DEFAULT_AUTH_NAME);
		...
		assertThat(token.getTokenValue()).isEqualTo(WithMockAccessTokenSecurityContextFactory.DEFAULT_TOKEN_VALUE);
		...
		assertThat(attributes.get(OAuth2IntrospectionClaimNames.USERNAME))
				.isEqualTo(WithMockAccessToken.DEFAULT_AUTH_NAME);
	}
