 ...
 * @since 5.2.0
 */
public class AccessTokenRequestPostProcessorTest extends AbstractRequestPostProcessorTest {

	@Test
	@SuppressWarnings("unchecked")
	public void test() {
		final AccessTokenRequestPostProcessor rpp = mockAccessToken().name(TEST_NAME)
		...
		final OAuth2IntrospectionAuthenticationToken actual =
				(OAuth2IntrospectionAuthenticationToken) authentication(rpp.postProcessRequest(request));
		...
		assertThat((Collection<String>) actual.getTokenAttributes().get(SCOPE_CLAIM_NAME))
